...
#ifndef HDSA_MD_DATA_INTERFACE_SYNTHETIC_TEST_OED_HPP
#define HDSA_MD_DATA_INTERFACE_SYNTHETIC_TEST_OED_HPP

#include "HDSA_MD_Data_Interface.hpp"
#include "HDSA_Std_Vector.hpp"

template <class RealT> class MD_Data_Interface_synthetic_test_OED : public HDSA::MD_Data_Interface<RealT> {
...
public:
  MD_Data_Interface_synthetic_test_OED(const HDSA::Ptr<HDSA::Random_Number_Generator<RealT>>& random_number_generator,
                                       const HDSA::Ptr<const HDSA::Comm<int>>& comm)
  ...
  virtual ~MD_Data_Interface_synthetic_test_OED() {}

  HDSA::Ptr<HDSA::Vector<RealT>> Load_Optimal_u() const {
    HDSA::Ptr<HDSA::Std_Vector<RealT>> u_opt =
        HDSA::makePtr<HDSA::Std_Vector<RealT>>(m_, random_number_generator_, comm_);
    for (int k = 0; k < m_; k++) {
  ...
  HDSA::Ptr<HDSA::Vector<RealT>> Load_Optimal_z() const {
    HDSA::Ptr<HDSA::Std_Vector<RealT>> z_opt =
        HDSA::makePtr<HDSA::Std_Vector<RealT>>(m_, random_number_generator_, comm_);
    for (int k = 0; k < m_; k++) {
  ...
  void Evaluate_Discrepancy(HDSA::Vector<RealT>& D_out, const HDSA::Vector<RealT>& Z_in) const {
    HDSA_TEST_FOR_EXCEPTION(D_out.Dimension() != Z_in.Dimension(), std::logic_error,
                            "Error in MD_Data_Interface_synthetic_test_OED::Evaluate_Discrepancy: "
                            "D_out and Z_in dimensions are inconsistent."
                                << std::endl);

    for (int k = 0; k < Z_in.Dimension(); ++k) {
      const RealT z_k = Z_in.Get_Entry(k);
      D_out.Set_Entry(k, static_cast<RealT>(0.2) * z_k * z_k * z_k);
    }
  }

  HDSA::Ptr<HDSA::Vector<RealT>> Evaluate_Discrepancy(const HDSA::Vector<RealT>& Z_in) const {
    HDSA::Ptr<HDSA::Vector<RealT>> D_out = Z_in.Clone();
    Evaluate_Discrepancy(*D_out, Z_in);
    return D_out;
  }
};
