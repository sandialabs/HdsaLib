...
#ifndef HDSA_MD_U_PRIOR_INTERFACE_SYNTHETIC_TEST_OED_HPP
#define HDSA_MD_U_PRIOR_INTERFACE_SYNTHETIC_TEST_OED_HPP

#include "HDSA_Linear_Algebra.hpp"
#include "HDSA_MD_u_Prior_Interface.hpp"

template <class RealT> class MD_u_Prior_Interface_synthetic_test_OED : public HDSA::MD_u_Prior_Interface<RealT> {
...
public:
  MD_u_Prior_Interface_synthetic_test_OED(
      const HDSA::Ptr<HDSA::Random_Number_Generator<RealT>>& random_number_generator)
  ...
  virtual ~MD_u_Prior_Interface_synthetic_test_OED() {}
  ...
  void Apply_W_u_Plus_scalar_M_u_Inverse(HDSA::Vector<RealT>& u_out, const HDSA::Vector<RealT>& u_in,
                                         const RealT& scalar) const {
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> b = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
    const HDSA::Std_Vector<RealT>& u_in_std = dynamic_cast<const HDSA::Std_Vector<RealT>&>(u_in);
    HDSA::Std_Vector<RealT>& u_out_std = dynamic_cast<HDSA::Std_Vector<RealT>&>(u_out);
    for (int k = 0; k < m_; k++) {
  ...
  // Compute samples from a mean zero Gaussian with covariance W_u^{-1}
  virtual void Sample_with_Covariance_W_u_Plus_scalar_M_u_Inverse(HDSA::MultiVector<RealT>& samples,
                                                                  const RealT& scalar) const {
  ...
  virtual HDSA::Ptr<HDSA::Dense_Matrix<RealT>> Get_W_u_Generalized_Eigenvalues() const {
    ...
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> lambda_js = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
    HDSA::Linear_Algebra::Symmetric_Gen_Eig_Decomposition<RealT>(*W_u_, *M_, *V, *lambda_js);
    return lambda_js;
  }
};
