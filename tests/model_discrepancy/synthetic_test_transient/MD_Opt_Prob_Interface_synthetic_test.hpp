...
public:
  MD_Opt_Prob_Interface_synthetic_test(HDSA::Ptr<const HDSA::Comm<int>> &comm, int n_y, int n_t, RealT c_low)
  {
    n_y_ = 51;
    n_t_ = n_t;
    c_low_ = c_low;
  ...
  void Apply_RS_Hessian(HDSA::Vector<RealT> &z_out, const HDSA::Vector<RealT> &z_in, const HDSA::Vector<RealT> &z) const
  {
    ...
    for (int k = 0; k < n_y_; k++)
    {
      RealT val = std::pow(c_low_, n_t_ - 1) * 3.0 * (z_in_view[k] * std::pow(z_view[k], 2.0));
      z_tmp1_tpetra.getVector()->replaceGlobalValue(k, 0, val);
    }
    M_sm_->Apply(*z_tmp2, *z_tmp1);
    for (int k = 0; k < n_y_; k++)
    {
      RealT val = std::pow(c_low_, n_t_ - 1) * 3.0 * z_tmp2_view[k] * std::pow(z_view[k], 2.0);
      z_out_tpetra.getVector()->replaceGlobalValue(k, 0, val);
    }
  }
  ...
  void Misfit_Gradient(HDSA::Vector<RealT> &u_grad, const HDSA::Vector<RealT> &u, const HDSA::Vector<RealT> &z) const
  {
    ...
    HDSA::Ptr<HDSA::Vector<RealT>> uf = u_trans[n_t_ - 1];
    ...
    for (int k = 0; k < n_y_; k++)
    {
      RealT val = uf_view[k] - std::pow(c_low_, n_t_ - 1) * std::pow((*x_)(k, 0) + 1.0, 3.0);
      uf_tmp_tpetra.getVector()->replaceGlobalValue(k, 0, val);
    }
