...
public:
  ...
  void Set_State_Component(HDSA::Vector<RealT> &u, const HDSA::Vector<RealT> &u_component, int component_id) const override
  {
    ...
    for (int k = 0; k < n_y_; k++)
    {
      u_tpetra.getVector()->replaceGlobalValue(component_id * n_y_ + k, 0, u_component_view[k]);
    }
  }
  ...
  HDSA::Ptr<HDSA::Vector<RealT>> Load_Optimal_u() const override
  {
    ...
    for (int j = 0; j < n_t_; j++)
    {
      ...
      for (int k = 0; k < n_y_; k++)
      {
        ...
        uj_tpetra.getVector()->replaceGlobalValue(n_y_ + k, 0, c_low_ * coeff * std::pow((*x_)(k, 0) + 1.0, 3.0));
      }
      coeff = c_low_ * coeff;
    }
    return u_opt;
  ...
  HDSA::Ptr<HDSA::MultiVector<RealT>> Load_D_Data() const override
  {
    ...
    for (int j = 0; j < n_t_; j++)
    {
      ...
      for (int k = 0; k < n_y_; k++)
      {
        ...
        d0_tpetra.getVector()->replaceGlobalValue(n_y_ + k, 0, (c_high_ * coeff_high - c_low_ * coeff_low) * std::pow((*x_)(k, 0) + 1.0, 3.0));
        d1_tpetra.getVector()->replaceGlobalValue(n_y_ + k, 0, (c_high_ * coeff_high - c_low_ * coeff_low) * std::pow((*x_)(k, 0) + std::pow((*x_)(k, 0), 2.0), 3.0));
      }
