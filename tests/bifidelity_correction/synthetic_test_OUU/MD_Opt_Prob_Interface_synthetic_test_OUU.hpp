...
template <class RealT> class MD_Opt_Prob_Interface_synthetic_test_OUU : public HDSA::MD_OUU_Opt_Prob_Interface<RealT> {
...
public:
  MD_Opt_Prob_Interface_synthetic_test_OUU(int ens_size, HDSA::Ptr<HDSA::Dense_Matrix<RealT>>& Xi)
      : HDSA::MD_OUU_Opt_Prob_Interface<RealT>(ens_size), Xi_(Xi) {
    m_ = 51;
  ...
  virtual ~MD_Opt_Prob_Interface_synthetic_test_OUU() {}
  ...
  // Assume a high-fidelity model u = Xi(0) * z^3 + Xi(1) + 0.2 * z^2

  void Apply_Solution_Operator_z_Jacobian_Transpose_Per_Sample(HDSA::Vector<RealT>& z_out,
                                                               const HDSA::Vector<RealT>& u_in,
                                                               const HDSA::Vector<RealT>& z, int s) const {
    ...
    for (int k = 0; k < m_; k++) {
      z_out_std.Set_Entry(k, (*Xi_)(0, s) * 3.0 * std::pow(z_std(k), 2.0) * u_in_std(k));
    }
  }

  void Apply_RS_Hessian_Per_Sample(HDSA::Vector<RealT>& z_out, const HDSA::Vector<RealT>& z_in,
                                   const HDSA::Vector<RealT>& z, int s) const {
    const HDSA::Std_Vector<RealT>& z_std = dynamic_cast<const HDSA::Std_Vector<RealT>&>(z);
    ...
    this->State_Solve_Per_Sample(*u, z, s);
    HDSA::Ptr<HDSA::Vector<RealT>> grad_u = z.Clone();
    this->Misfit_Gradient_Per_Sample(*grad_u, *u, z, s);
    ...
    for (int k = 0; k < m_; ++k) {
      v->Set_Entry(k, 0, (*Xi_)(0, s) * 3.0 * std::pow(z_std(k), 2.0) * z_in_std(k));
    }
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> M_v = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
    M_->Multiply(*M_v, *v);
    for (int k = 0; k < m_; ++k) {
      const RealT gauss_newton_term = (*Xi_)(0, s) * 3.0 * (*M_v)(k, 0) * std::pow(z_std(k), 2.0);
      const RealT second_order_term = (*Xi_)(0, s) * 6.0 * z_std(k) * z_in_std(k) * grad_u_std(k);
      z_out_std.Set_Entry(k, gauss_newton_term + second_order_term);
    }
  }

  void Misfit_Gradient_Per_Sample(HDSA::Vector<RealT>& u_grad, const HDSA::Vector<RealT>& u,
                                  const HDSA::Vector<RealT>& z, int s) const {
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> v = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
  ...
  void Apply_Misfit_Hessian_Per_Sample(HDSA::Vector<RealT>& u_out, const HDSA::Vector<RealT>& u_in,
                                       const HDSA::Vector<RealT>& u, const HDSA::Vector<RealT>& z, int s) const {
    HDSA::Ptr<HDSA::Dense_Matrix<RealT>> v = HDSA::makePtr<HDSA::Dense_Matrix<RealT>>(m_, 1);
    const HDSA::Std_Vector<RealT> u_in_std = dynamic_cast<const HDSA::Std_Vector<RealT>&>(u_in);
  ...
  void State_Solve_Per_Sample(HDSA::Vector<RealT>& u_out, const HDSA::Vector<RealT>& z, int s) const {
    const HDSA::Std_Vector<RealT> z_std = dynamic_cast<const HDSA::Std_Vector<RealT>&>(z);
    HDSA::Std_Vector<RealT>& u_out_std = dynamic_cast<HDSA::Std_Vector<RealT>&>(u_out);
    for (int k = 0; k < m_; k++) {
      u_out_std.Set_Entry(k, (*Xi_)(0, s) * std::pow(z_std(k), 3.0) + (*Xi_)(1, s));
    }
  }
};
