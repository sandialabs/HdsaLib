...
template <class RealT, class LO = Tpetra::Map<>::local_ordinal_type, class GO = Tpetra::Map<>::global_ordinal_type, class Node = Tpetra::Map<>::node_type> class Driver_MrHyDE
{
  ...
  public:
    Driver_MrHyDE(Teuchos::RCP<MpiComm> &comm, Teuchos::RCP<Teuchos::ParameterList> &settings, Teuchos::RCP<MrHyDE::SolverManager<SolverNode>> &solver, Teuchos::RCP<MrHyDE::PostprocessManager<SolverNode>> &postproc,
    ...
    void BF_Solve(Teuchos::ParameterList &HDSAsettings)
    {
        ...
        {
            random_number_generator = HDSA::makePtr<HDSA::Random_Number_Generator<RealT>>(hdsa_comm);
        }
        else
        {
            ...
            random_number_generator = HDSA::makePtr<HDSA::Random_Number_Generator<RealT>>(num_random_numbers, random_number_file);
        }
        ...
        if (hdsa_verbosity > 1)
        {
            HDSA::Ptr<const HDSA::Vector<RealT>> u_opt = data_interface->Get_u_opt();
            HDSA::Ptr<const HDSA::Vector<RealT>> z_opt = data_interface->Get_z_opt();
        ...
        HDSA::Ptr<HDSA::BF_Sol_Op_Interface<RealT>> sol_op_interface = HDSA::makePtr<BF_Sol_Op_Interface_MrHyDE<RealT>>(comm_, solver_->varlist, random_number_generator);
        ...
        HDSA::Ptr<Output_Writer_MrHyDE<RealT>> output_writer = HDSA::makePtr<Output_Writer_MrHyDE<RealT>>(postproc_, solver_, write_exo);
    ...
    void MD_Solve(Teuchos::ParameterList &HDSAsettings)
    {
        ...
        for (int k = 0; k < num_states; k++)
        {
            ...
            alpha_u[k] = HDSAsettings.sublist("HyperParameters").sublist(state_var_name).get<RealT>("alpha_u", 0.0);
        ...
        RealT alpha_z = HDSAsettings.sublist("HyperParameters").sublist("z").get<RealT>("alpha_z", 0.0);
        RealT beta_z = HDSAsettings.sublist("HyperParameters").sublist("z").get<RealT>("beta_z", 0.0);
        RealT beta_t_z_prior = HDSAsettings.sublist("HyperParameters").sublist("z").get<RealT>("beta_t", 0.0);

        RealT max_marginal_var_percent = HDSAsettings.sublist("HyperParameters").sublist("OUU").get<RealT>("max_marginal_var_percent", 1.0);
        RealT min_cond_variance_percent = HDSAsettings.sublist("HyperParameters").sublist("OUU").get<RealT>("min_cond_variance_percent", 0.1);
        ...
        HDSA::Ptr<ROL::SampleGenerator<RealT>> sampler;
        ...
        if (is_stoch)
        {
            ...
            HDSA::Ptr<ROL::BatchManager<RealT>> bman = HDSA::makePtr<ROL::MrHyDETeuchosBatchManager<RealT, int>>(comm_);
            ...
            sampler = HDSA::makePtr<ROL::Sample_Set_Reader<RealT>>(ens_size, stoch_dim, bman, sample_pt_file, sample_wt_file);
        }
        ...
        if (is_stoch)
        {
            std::vector<HDSA::Ptr<MD_Data_Interface_MrHyDE<RealT>>> data_interface_ens;
            data_interface_ens.resize(ens_size);
            for (int s = 0; s < ens_size; s++)
            {
                data_interface_ens[s] = HDSA::makePtr<MD_Data_Interface_MrHyDE<RealT>>(comm_, solver_, params_, random_number_generator, data_load_list);
            ...
            data_interface = HDSA::makePtr<MD_OUU_Data_Interface_MrHyDE<RealT>>(data_interface_ens, sampler, ens_size);
        }
        else
        {
            data_interface = HDSA::makePtr<MD_Data_Interface_MrHyDE<RealT>>(comm_, solver_, params_, random_number_generator, data_load_list);
        }
        ...
        if (hdsa_verbosity > 0)
        {
            HDSA::Ptr<const HDSA::Vector<RealT>> u_opt = data_interface->Get_u_opt();
            HDSA::Ptr<const HDSA::Vector<RealT>> z_opt = data_interface->Get_z_opt();
            HDSA::Ptr<const HDSA::MultiVector<RealT>> Z = data_interface->Get_Z();
            HDSA::Ptr<const HDSA::MultiVector<RealT>> D = data_interface->Get_D();
            *outStream << "u_opt->norm() = " << u_opt->Norm() << std::endl;
            *outStream << "z_opt->norm() = " << z_opt->Norm() << std::endl;
        ...
        if (is_stoch)
        {
            std::vector<RealT> ens_weights = std::vector<RealT>(ens_size, 0.0);
            for (int s = 0; s < ens_size; s++)
            {
                ens_weights[s] = sampler->getMyWeight(s);
            }
            opt_prob_interface = HDSA::makePtr<MD_OUU_Opt_Prob_Interface_MrHyDE<RealT>>(solver_, postproc_, params_, data_interface, sampler, ens_weights);
        }
        else
        {
            opt_prob_interface = HDSA::makePtr<MD_Opt_Prob_Interface_MrHyDE<RealT>>(solver_, postproc_, params_, data_interface);
        }
        ...
        std::vector<HDSA::Ptr<HDSA::MD_u_Hyperparameter_Interface<RealT>>> u_hyperparam_interface_std;
        std::vector<HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>>> u_prior_interface_std;
        ...
        for (int k = 0; k < num_states; k++)
        {
            ...
            if (is_transient)
            {
                HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>> spatial_u_prior_interface_k;
                HDSA::Ptr<HDSA::MD_Transient_Prior_Covariance<RealT>> transient_prior_cov_k;
                RealT T = solver_->final_time;
                int n_t = solver_->settings->sublist("Solver").get<int>("number of steps", 0) + 1;

                if (is_stoch)
                {
                    HDSA::Ptr<HDSA::MD_OUU_Data_Interface<RealT>> ouu_data_interface = Teuchos::rcp_dynamic_cast<HDSA::MD_OUU_Data_Interface<RealT>>(data_interface);
                ...
                u_prior_interface_std[k] = HDSA::makePtr<HDSA::MD_Transient_Elliptic_u_Prior_Interface<RealT>>(spatial_u_prior_interface_k, transient_prior_cov_k);
            }
            else
            {
                if (is_stoch)
        ...
        if (is_stoch)
        {
            HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>> us_prior_interface;
            if (num_states > 1)
            {
                u_hyperparam_interface = HDSA::makePtr<HDSA::MD_Multi_State_u_Hyperparameter_Interface<RealT>>(u_hyperparam_interface_std);
                us_prior_interface = HDSA::makePtr<HDSA::MD_Multi_State_u_Prior_Interface<RealT>>(data_interface, u_prior_interface_std);
            }
            else
            {
            ...
            HDSA::Ptr<HDSA::MD_OUU_Ensemble_Weighting_Matrix<RealT>> ensemble_weighting =
                HDSA::makePtr<HDSA::MD_OUU_Ensemble_Weighting_Matrix<RealT>>(data_interface, us_prior_interface, ens_size, max_marginal_var_percent, min_cond_variance_percent, assume_independent_ensembles);
            u_prior_interface = HDSA::makePtr<HDSA::MD_OUU_u_Prior_Interface<RealT>>(us_prior_interface, ensemble_weighting);
        }
        else
        {
            if (num_states > 1)
            {
                u_hyperparam_interface = HDSA::makePtr<HDSA::MD_Multi_State_u_Hyperparameter_Interface<RealT>>(u_hyperparam_interface_std);
                u_prior_interface = HDSA::makePtr<HDSA::MD_Multi_State_u_Prior_Interface<RealT>>(data_interface, u_prior_interface_std);
            }
            else
            {
                u_hyperparam_interface = u_hyperparam_interface_std[0];
        ...
        HDSA::Ptr<HDSA::MD_z_Hyperparameter_Interface<RealT>> z_hyperparam_interface =
            HDSA::makePtr<MD_z_Hyperparameter_Interface_MrHyDE<RealT>>(solver_, params_, comm_, data_interface, random_number_generator, z_type, prior_num_state_solves);
        ...
        else if (z_type == "transient vector")
        {
            ...
            z_prior_interface = HDSA::makePtr<HDSA::MD_Transient_Vector_z_Prior_Interface<RealT>>(data_interface, z_hyperparam_interface, u_prior_interface, n_t, T, num_controls);
        }
        else if (z_type == "vector")
        {
            z_prior_interface = HDSA::makePtr<HDSA::MD_Vector_z_Prior_Interface<RealT>>(data_interface, z_hyperparam_interface, u_prior_interface);
        }
        ...
        if ((num_prior_samples > 0) & execute_prior_discrepancy_sampling)
        {
            ...
            HDSA::Ptr<HDSA::MultiVector<RealT>> spatial_coords = data_interface->Read_Spatial_Node_Data();
            ...
            if (is_transient)
            {
                std::vector<std::vector<std::vector<RealT>>> prior_delta_z_opt_time_evol = prior_sampling->Get_prior_delta_z_opt_time_evol();
                std::vector<std::vector<RealT>> prior_discrep_data_time_evol = prior_sampling->Get_prior_discrep_data_time_evol();
        ...
        if ((num_posterior_samples > 0) & execute_posterior_discrepancy_sampling)
        {
            ...
            std::vector<HDSA::Ptr<HDSA::Vector<RealT>>> z_in;
            ...
            std::vector<HDSA::Ptr<HDSA::MD_Posterior_Vectors<RealT>>> post_delta = post_sampling->Posterior_Discrepancy_Samples(z_in);

            for (int k = 0; k < N; k++)
            {
                HDSA::Ptr<HDSA::Vector<RealT>> dk = (*data_interface->Get_D())[k];
                HDSA::Ptr<HDSA::Vector<RealT>> tmp1 = dk->Clone();
                HDSA::Ptr<HDSA::Vector<RealT>> tmp2 = dk->Clone();
        ...
        if (execute_optimal_solution_update)
        {
            ...
            HDSA::Ptr<HDSA::MD_Hessian_Analysis<RealT>> hessian_analysis = HDSA::makePtr<HDSA::MD_Hessian_Analysis<RealT>>(opt_prob_interface, z_prior_interface, hdsa_verbosity, hessian_tol, hessian_solver);
            if (hessian_num_eig_vals > 0)
            {
                ...
                HDSA::Ptr<HDSA::Dense_Matrix<RealT>> evals = hessian_analysis->Get_Evals();
                output_writer->Write_Hessian_Eigenvalues(evals);
            }

            HDSA::Ptr<HDSA::MD_Update<RealT>> update =
                HDSA::makePtr<HDSA::MD_Update<RealT>>(data_interface, u_prior_interface, z_prior_interface, opt_prob_interface, post_sampling, hessian_analysis, random_number_generator, num_continuation_steps);
            ...
            {
                HDSA::Ptr<HDSA::Vector<RealT>> z_update_mean = update->Posterior_Update_Mean();
