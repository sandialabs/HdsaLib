...
template <class RealT, class LO = Tpetra::Map<>::local_ordinal_type, class GO = Tpetra::Map<>::global_ordinal_type, class Node = Tpetra::Map<>::node_type> class Prior_Operators_Interface_MrHyDE
{
  ...
  public:
    ...
    Teuchos::RCP<Tpetra::CrsMatrix<ScalarT, LO, GO, Node>> Instantiate_Prior_Operators(Teuchos::RCP<Teuchos::MpiComm<int>> comm, Teuchos::RCP<Teuchos::ParameterList> &Settings, std::vector<string> &blockNames)
    {
        ...
        Teuchos::RCP<MrHyDE::PhysicsInterface> physics =
            Teuchos::rcp(new MrHyDE::PhysicsInterface(Settings, comm, mesh->getBlockNames(), mesh->getPhaseBlockNames(), mesh->getSideNames(), mesh->getDimension(), mesh->getPhaseDimension()));

        mesh->finalize(physics->getVarList(), physics->getVarTypes(), physics->getDerivedList());

        Teuchos::RCP<MrHyDE::DiscretizationInterface> disc = Teuchos::rcp(new MrHyDE::DiscretizationInterface(Settings, comm, mesh, physics));
        ...
        Teuchos::RCP<MrHyDE::MultiscaleManager> multiscale_manager = Teuchos::rcp(new MrHyDE::MultiscaleManager(comm, mesh, Settings, assembler->groups,
#ifndef MrHyDE_NO_AD
        ...
        Teuchos::RCP<MrHyDE::PostprocessManager<SolverNode>> postproc = Teuchos::rcp(new MrHyDE::PostprocessManager<SolverNode>(comm, Settings, mesh, disc, physics, multiscale_manager, assembler, params));
        ...
        assembler->assembleJacRes(set, stage, zero_soln, zero_soln, zero_soln, zero_soln, zero_soln, zero_soln, true, false, false, false, 0, current_res_over, J_over, false, 0.0, false, false, params->num_active_params,
                                  paramvec, paramdot, false, 0.0);
        solve->linalg->fillComplete(J_over);
        J->resumeFill();
