...
public:
  ...
  static RealT Evaluate_Max_Violation(const HDSA::Dense_Matrix<RealT>& beta, const HDSA::Dense_Matrix<RealT>& beta_bar,
                                      const RealT radius, const HDSA::Dense_Matrix<RealT>& K) {
    const int r = DV::Length(beta_bar);
    const int beta_len = DV::Length(beta);

    HDSA_TEST_FOR_EXCEPTION(r <= 0, std::logic_error,
                            "Error in HDSA::MD_Ellipsoid_Ball_SPG::Evaluate_Max_Violation: "
                            "beta_bar must be nonempty."
                                << std::endl);

    HDSA_TEST_FOR_EXCEPTION(beta_len <= 0, std::logic_error,
                            "Error in HDSA::MD_Ellipsoid_Ball_SPG::Evaluate_Max_Violation: "
                            "beta must contain at least one reduced design column."
                                << std::endl);

    HDSA_TEST_FOR_EXCEPTION(beta_len % r != 0, std::logic_error,
                            "Error in HDSA::MD_Ellipsoid_Ball_SPG::Evaluate_Max_Violation: "
                            "beta length must be divisible by r = length(beta_bar)."
                                << std::endl);
    ...
    const int p = beta_len / r;

    const bool matrix_layout = beta.Number_of_Rows() == r && beta.Number_of_Columns() == p;
    ...
    const RealT radius_sq = radius * radius;

    RealT max_violation = -std::numeric_limits<RealT>::infinity();

    std::vector<RealT> diff(r);

    for (int col = 0; col < p; ++col) {
      for (int i = 0; i < r; ++i) {
        diff[i] = get_coeff(beta, i, col) - DV::Get_Column_Major(beta_bar, i);
      }

      RealT quad = static_cast<RealT>(0);

      for (int i = 0; i < r; ++i) {
        RealT Kdiff_i = static_cast<RealT>(0);

        for (int j = 0; j < r; ++j) {
          Kdiff_i += K(i, j) * diff[j];
        }

        quad += diff[i] * Kdiff_i;
      }

      max_violation = std::max(max_violation, quad - radius_sq);
    }

    HDSA_TEST_FOR_EXCEPTION(!std::isfinite(max_violation), std::logic_error,
                            "Error in HDSA::MD_Ellipsoid_Ball_SPG::Evaluate_Max_Violation: "
                            "Computed nonfinite violation."
                                << std::endl);
