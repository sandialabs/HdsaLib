...
namespace HDSA
{
  ...
  public:
    MD_Update(const HDSA::Ptr<HDSA::MD_Data_Interface<RealT>> &data_interface, const HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>> &u_prior_interface,
              ...
              const HDSA::Ptr<HDSA::Random_Number_Generator<RealT>> &random_number_generator, int num_continuation_steps = 0, const RealT grad_tol = 1e-5, bool discard_cache = true) : num_continuation_steps_(num_continuation_steps)
    {
      if (num_continuation_steps_ > 0)
      {
        continuation_update_ = HDSA::makePtr<HDSA::MD_Continuation_Update<RealT>>(data_interface, u_prior_interface, z_prior_interface, opt_prob_interface, post_sampling, hessian_analysis, random_number_generator, num_continuation_steps, grad_tol, discard_cache);
      }
      else
      {
