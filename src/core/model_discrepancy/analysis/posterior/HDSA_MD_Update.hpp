...
namespace HDSA
{
  ...
  public:
    MD_Update(const HDSA::Ptr<HDSA::MD_Data_Interface<RealT>> &data_interface, const HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>> &u_prior_interface,
              ...
              const HDSA::Ptr<HDSA::MD_Posterior_Sampling<RealT>> &post_sampling, const HDSA::Ptr<HDSA::MD_Hessian_Analysis<RealT>> &hessian_analysis, int num_continuation_steps = 0) : num_continuation_steps_(num_continuation_steps)
    {
      if (num_continuation_steps_ > 0)
      {
        continuation_update_ = HDSA::makePtr<HDSA::MD_Continuation_Update<RealT>>(data_interface, z_prior_interface, opt_prob_interface, post_sampling, hessian_analysis, num_continuation_steps);
      }
      else
      {
