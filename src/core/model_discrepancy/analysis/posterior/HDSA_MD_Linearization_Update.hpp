...
#ifndef HDSA_MD_LINEARIZATION_UPDATE_HPP
#define HDSA_MD_LINEARIZATION_UPDATE_HPP

namespace HDSA
{

  template <class RealT>
  class MD_Linearization_Update
  {
  ...
  public:
    MD_Linearization_Update(const HDSA::Ptr<HDSA::MD_Data_Interface<RealT>> &data_interface, const HDSA::Ptr<HDSA::MD_u_Prior_Interface<RealT>> &u_prior_interface,
    ...
    HDSA::Ptr<HDSA::MD_Posterior_Vectors<RealT>> Posterior_Update_Samples(void) const
    {
      ...
      for (int i = 0; i < post_sampling_->post_data->N; i++)
      {
        ...
        M_z_W_z_inv_M_z_yi->Scaled_Plus(-post_sampling_->post_data->sum_g_vecs[i], *post_sampling_->post_data->M_z_W_z_inv_M_z_z_opt);
        for (int j = 0; j < post_sampling_->post_data->N; j++)
        {
          M_z_W_z_inv_M_z_yi->Scaled_Plus((*post_sampling_->post_data->g_vecs)(j, i), *(*post_sampling_->post_data->M_z_W_z_inv_M_z_Z)[j]);
        }
      ...
      if (post_sampling_->post_data->N > 1)
      {
        ...
        for (int k = 0; k < post_sampling_->post_data->num_samples; k++)
        {
          ...
          {
            (*B_theta_breve)[k]->Scaled_Plus(-(*tmp2)(i, k), *(*post_sampling_->post_data->M_z_W_z_inv_M_z_Zc)[i]);
          }
        }
      }
    ...
    HDSA::Ptr<HDSA::Vector<RealT>> Posterior_Update_Mean(void) const
    {
      ...
      for (int ell = 0; ell < post_sampling_->post_data->N; ell++)
      {
        ...
        for (int i = 0; i < post_sampling_->post_data->N; i++)
        {
          ...
          u_tmp1->Scaled_Plus(-coeff, *(*post_sampling_->post_data->u_i_ell[i])[ell]);
        }
      }
      u_tmp1->Scaled_Plus(post_sampling_->post_data->alpha_d, *data_interface_->Get_data_shift());
      ...
      for (int ell = 0; ell < post_sampling_->post_data->N; ell++)
      {
        ...
        z_tmp1->Scaled_Plus(coeff1, *(*post_sampling_->post_data->M_z_W_z_inv_M_z_Z)[ell]);
        z_tmp1->Scaled_Plus(-coeff1, *post_sampling_->post_data->M_z_W_z_inv_M_z_z_opt);
        for (int i = 0; i < post_sampling_->post_data->N; i++)
        {
          RealT coeff2 = state_grad_->Dot(*(*post_sampling_->post_data->u_i_ell[i])[ell]);
          coeff2 *= (*post_sampling_->post_data->b_i_ell)(i, ell);

          z_tmp1->Scaled_Plus(coeff2 * post_sampling_->post_data->sum_g_vecs[i], *post_sampling_->post_data->M_z_W_z_inv_M_z_z_opt);
          for (int j = 0; j < post_sampling_->post_data->N; j++)
          {
            z_tmp1->Scaled_Plus(-coeff2 * (*post_sampling_->post_data->g_vecs)(j, i), *(*post_sampling_->post_data->M_z_W_z_inv_M_z_Z)[j]);
          }
        }
      }
