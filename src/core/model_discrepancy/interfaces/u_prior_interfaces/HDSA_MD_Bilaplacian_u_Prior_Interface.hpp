/***********************************************************************
 HdsaLib - A library for
Hyper-differential Sensitivity
Analysis

 Questions? Contact Joseph
Hart (joshart@sandia.gov)
************************************************************************/

#ifndef HDSA_MD_BILAPLACIAN_U_PRIOR_INTERFACE_HPP
#define HDSA_MD_BILAPLACIAN_U_PRIOR_INTERFACE_HPP
...
template <class RealT> class MD_Bilaplacian_u_Prior_Interface : public HDSA::MD_Scaled_u_Prior_Interface<RealT>
{
  ...
  private:
    ...
    HDSA::Ptr<HDSA::Sparse_Matrix_Solver<RealT>> M_u_solver_;
    HDSA::Ptr<HDSA::Sparse_Matrix_Sqrt<RealT>> M_u_sqrt_;
  ...
  public:
    ...
    {
        HDSA::Ptr<HDSA::Vector<RealT>> u_tmp1 = u_in.Clone();
        Apply_E_u_Inverse(*u_tmp1, u_in);
        HDSA::Ptr<HDSA::Vector<RealT>> u_tmp2 = u_in.Clone();
        Apply_M_u(*u_tmp2, *u_tmp1);
        Apply_E_u_Inverse(u_out, *u_tmp2);
    }
    ...
    void Apply_W_u_Acute_Plus_scalar_M_u_Inverse(HDSA::Vector<RealT> &u_out, const HDSA::Vector<RealT> &u_in, const RealT &scalar) const override
    {
        ...
        if (i < 0)
        {
            HDSA_TEST_FOR_EXCEPTION(true, std::logic_error,
                                    "Error in HDSA::MD_Bilaplacian_u_Prior_Interface::Apply_W_u_Acute_Plus_scalar_M_u_Inverse: scalar has "
                                    "not been set by Precompute_W_u_Plus_scalar_M_u_Data"
                                        << std::endl);
        }
    ...
    void Sample_with_Covariance_W_u_Acute_Inverse(HDSA::MultiVector<RealT> &samples) const override
    {
        ...
        for (int k = 0; k < num_samples; k++)
        {
            ...
            std::string output_message = M_u_sqrt_->Matrix_Sqrt_Apply(*vec, *omega);
            if (verbosity_ > 3)
            {
                RealT elapsed_time = timer_->End_Timer();
                out_stream_ << output_message << " in " << elapsed_time << " seconds." << std::endl;
    ...
    void Sample_with_Covariance_W_u_Acute_Plus_scalar_M_u_Inverse(HDSA::MultiVector<RealT> &samples, const RealT &scalar) const override
    {
        ...
        if (i < 0)
        {
            HDSA_TEST_FOR_EXCEPTION(true, std::logic_error,
                                    "Error in "
                                    "HDSA::MD_Bilaplacian_u_Prior_Interface::Sample_with_Covariance_W_u_Acute_Plus_"
                                    "scalar_M_u_Inverse: scalar has not been set by Precompute_W_u_Plus_scalar_M_u_Data"
                                        << std::endl);
        }
    ...
    void Precompute_W_u_Plus_scalar_M_u_Data(RealT &scalar) override
    {
        ...
        HDSA::Ptr<HDSA::Linear_Operator<RealT>> A_op = HDSA::makePtr<Shifted_Bilaplacian_Operator<RealT>>(this, scalar_shift);
        A_op->Set_Symmetric();
        std::string A_solver_message = "W_u_acute_plus_scalar_M_u_Inverse";
        HDSA::Ptr<HDSA::Sparse_Matrix_Solver<RealT>> A_solver = HDSA::makePtr<HDSA::Sparse_Matrix_Solver<RealT>>(A_op, verbosity_, out_stream_, A_solver_message);
        std::string A_sqrt_solver_message = "W_u_acute_plus_scalar_M_u_Sqrt";
        HDSA::Ptr<HDSA::Sparse_Matrix_Sqrt<RealT>> A_sqrt = HDSA::makePtr<HDSA::Sparse_Matrix_Sqrt<RealT>>(A_op, A_sqrt_solver_message);
    ...
    MD_Bilaplacian_u_Prior_Interface(const HDSA::Ptr<HDSA::Sparse_Matrix<RealT>> &S, const HDSA::Ptr<HDSA::Sparse_Matrix<RealT>> &M, const HDSA::Ptr<HDSA::MD_Data_Interface<RealT>> &data_interface,
          ...
          use_direct_solvers_(use_direct_solvers), verbosity_(verbosity), use_incomplete_prec_(use_incomplete_prec), out_stream_(out_stream)
    {
        ...
        if (use_incomplete_prec_)
        {
            ...
            {
                RealT elaped_time = timer_->End_Timer();
                out_stream_ << "M_u incomplete factorization took " << elaped_time << " seconds." << std::endl;
            }
        }
        ...
        {
            HDSA_TEST_FOR_EXCEPTION(true, std::logic_error, "Error in HDSA::MD_Numeric_Bilaplacian_u_Prior_Interface: The value of beta_u must be specificed" << std::endl);
        }
    ...
    virtual ~MD_Bilaplacian_u_Prior_Interface()
    {
    }
    ...
    {
        int max_entries_per_row = 3 * E_u_->Get_Max_Nonzeros_Per_Row(); // This may fail in spatial dimensions > 2. Need
                                                                        // to test this and possibly modify.
    ...
    {
      private:
        const HDSA::MD_Bilaplacian_u_Prior_Interface<ScalarType> *bilapliacian_u_prior_interface_;
        const ScalarType scalar_;

      public:
        Shifted_Bilaplacian_Operator(const HDSA::MD_Bilaplacian_u_Prior_Interface<ScalarType> *bilapliacian_u_prior_interface, const ScalarType scalar)
            : bilapliacian_u_prior_interface_(bilapliacian_u_prior_interface), scalar_(scalar)
        {
        }

        ~Shifted_Bilaplacian_Operator()
        {
        }

        void Apply(HDSA::Vector<ScalarType> &y, const HDSA::Vector<ScalarType> &x) const
        {
            HDSA::Ptr<HDSA::Vector<RealT>> tmp1 = x.Clone();
            bilapliacian_u_prior_interface_->E_u_->Apply(*tmp1, x);
            HDSA::Ptr<HDSA::Vector<RealT>> tmp2 = x.Clone();
            bilapliacian_u_prior_interface_->M_u_solver_->Apply_A_Inverse(*tmp2, *tmp1);
            bilapliacian_u_prior_interface_->E_u_->Apply(y, *tmp2);

            tmp1->Zeros();
            bilapliacian_u_prior_interface_->M_->Apply(*tmp1, x);
            y.Scaled_Plus(scalar_, *tmp1);
        }
    };
};
} // namespace HDSA
